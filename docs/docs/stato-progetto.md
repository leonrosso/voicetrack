...
| Card azioni rapide sul Diario (testo / scan / voce → tab Traccia o Scan) | ✅ (codice; solo giorno corrente; nel fold sopra Macronutrienti e «Pasti di oggi») |
| Layout Diario above-fold + Obiettivi a tutta altezza viewport | ✅ (codice; ordine calorie → azioni → macro; padding basso `calc(22px + env(safe-area-inset-bottom))` + `viewport-fit=cover`; **hero a slot fissi** data | medio | gauge; readout centrato con fade; editor absolute nello slot medio; **data/gauge/readout tornano col gate `targetsAnimOpen`** (stesso atto di azioni/macro); collasso data+gauge in apertura; blocco azioni/macro via `flex-grow`; footer Salva/Annulla fisso; anti-flash scrollbar via lock html/body + `.vt-targets-scroll`) |
...
Metodo per il frontend: repo locale + `npm run dev` + anteprima live sul telefono; AI che modifica i file direttamente; deploy Vercel a fine rifinitura. Test su telefono reale, mai in artifact. **Limitazione PWA:** layout viewport / safe-area / gesture bar Android si verificano solo in modalità **standalone** (app installata); Chrome con barra indirizzi (`npm run dev`) maschera il problema. Altre limitazioni già note: Web Speech e camera non in anteprima sandbox.
...
- **Card azioni rapide sul Diario:** ✅ (codice) tra Macronutrienti e Pasti di oggi, tre bottoni rotondi (TESTO / SCAN / VOCE) visibili solo oggi; aprono i tab Traccia/Scan esistenti e avviano focus input, camera o microfono. Con calorie + macro formano il **fold** della prima viewport (`minHeight: calc(100dvh - 150px)`); «Pasti di oggi» inizia sotto lo scroll.
- **Obiettivi a tutta altezza:** ✅ (codice) con pannello Obiettivi aperto la card calorie cresce **verso il basso** nello spazio del fold (macro/azioni nascoste); bordo alto fermo sotto le tab; contenuto data/kcal ancorato in alto (`justifyContent: flex-start` fisso, niente salto). Fold con `paddingBottom: calc(22px + env(safe-area-inset-bottom))` e meta `viewport-fit=cover` (margine sotto Grassi anche in PWA standalone). Da verificare su telefono **installato**.
