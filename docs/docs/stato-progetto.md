...
DATABASE — Google Sheets (tab dati `diethropic` + tab `Config` + tab `Catalogo`)
  ...
  | grassi | fonte (tasker-voce / pwa-voce / pwa-testo / pwa-barcode / pwa-catalogo) | note | id
...
- **Sheets:** foglio condiviso col service account; tab `diethropic` + tab `Config` (auto-creato) + tab `Catalogo` (auto-creato al primo `/catalog` o `/search` / scan ok).
- **PWA:** `https://voicetrack-chi.vercel.app/`; URL backend + chiave in localStorage. CORS aperto (`*` — barriera = API key). Scan barcode: anche `vt-scan-camera-id` (deviceId back camera preferito).
...
| Cloud Function + tutti gli endpoint (`/log_meal`, `/daily_summary`, `/dashboard`, `/config`, `/health`, `/scan_barcode`, `/catalog`, `/search`, `/log_catalog`) | ✅ (catalogo/search/log_catalog in codice; da deployare) |
...
| Scan: EAN a mano con microfono (dettatura cifre → campo) | ✅ (codice; da verificare su telefono) |
| Catalogo personale + CERCA (frequenti/preferiti, OFF text search, log da catalogo) | ✅ (codice; **CERCA = overlay fisso**, non tab Traccia; **stellina Diario/CERCA = toggle preferiti** con stella piena + toast; cestino in CERCA per delete; upsert match anche per nome; da deployare CF match-nome + Vercel + test telefono/desktop) |
...
| Focus camera barcode (parte sulla lente 0.5x) | ✅ (codice; zoom hardware ~2 + `deviceId` in localStorage + AF continuo; zoom digitale 1.35; **da verificare su telefono**) |
| Colonna `id` + `/update_meal` + `/delete_meal` | ✅ (codice; da deployare sul Cloud Function) |
| Edit/delete pasti nella PWA (tap→modifica, pulsante+swipe→elimina; conferma su barra rossa inline, non tendina; pannello edit con animazione collapse altezza/opacità) | ✅ (codice; da deployare backend + test su telefono) |
...
| Card azioni rapide sul Diario (testo / cerca / scan / voce → tab Traccia o Scan) | ✅ (codice; visibile anche sui giorni storici; i log PWA usano il giorno attivo del Diario via `target_date`, fallback legacy su oggi se assente) |
| Layout Diario above-fold + Obiettivi a tutta altezza viewport | ✅ (codice; ordine calorie → azioni → macro; altezza via `--app-height`/`visualViewport` non `100dvh`; padding basso `max(40px, env(safe-area-inset-bottom))` + `viewport-fit=cover`; **hero a slot fissi** data | medio | gauge; readout centrato con fade; editor absolute nello slot medio; **data/gauge/readout tornano col gate `targetsAnimOpen`** (stesso atto di azioni/macro); collasso data+gauge in apertura; blocco azioni/macro via `flex-grow`; **principio unificato oggi/storico** sul contenitore azioni+macro (stesso `flexGrow`), per evitare macro più piccola nei giorni passati; footer Salva/Annulla fisso; anti-flash scrollbar via lock html/body + `.vt-targets-scroll`; **swipe dx/sx sulla card calorie** → carosello giorno tipo tab, peeks adiacenti; **contenimento `overflowX: clip` + `minWidth: 0`** anti doppia larghezza; **guard su end/cancel** + **flushSync transition none prima dello snap** anti rimbalzo settle) |
...
- **Schema Sheets = moltiplicatore:** i consumatori pasti sono ≥5 (`/log_meal`, `/scan_barcode`, `/daily_summary`, `/dashboard`, `/config`); il **Catalogo** è una tab separata (`/catalog`, `/search`, `/log_catalog`) e non allunga lo schema A–J dei pasti. **Ogni colonna aggiunta ai pasti si aggiorna in tutti i consumatori insieme, nella stessa sessione.** Nessuna validazione numerica: una riga malformata inquina i totali in silenzio. `pasto` derivato dall'orario è un'euristica fragile.
...
- **Edit/delete pasti:** tap → pannello modifica → `/update_meal`; swipe → conferma **sulla barra rossa della riga** (non tendina) → `/delete_meal`; refresh dashboard + invalidazione cache `vt-cache`.
...
- **Card azioni rapide sul Diario:** ✅ (codice) tra Macronutrienti e Pasti di oggi, quattro bottoni rotondi (TESTO / CERCA / SCAN / VOCE) visibili anche sui giorni storici; aprono i tab Traccia/Scan esistenti e avviano focus input, mode cerca, camera o microfono. I flussi di log PWA (`/log_meal`, `/scan_barcode`, `/log_catalog`) inviano `target_date` e restano ancorati al giorno attivo anche nei passaggi multi-step; se `target_date` manca (Tasker/client legacy) il backend continua a registrare su oggi. Con calorie + macro formano il **fold** della prima viewport; «Pasti di oggi» inizia sotto lo scroll.
...
- **Catalogo / CERCA:** ✅ (codice) tab Sheets `Catalogo`; `/catalog`, `/search`, `/log_catalog`; **overlay CERCA** dal bottone azioni rapide (non mode in Traccia); CTA «Aggiungi al catalogo» su Scan not_found; **stellina Diario/CERCA = toggle preferiti** (`star`/`unstar`, stella piena se preferito, toast «Aggiunto/Rimosso dai preferiti»); se non in catalogo la stellina Diario apre ancora overlay salva; **cestino in CERCA** → `action=delete`; upsert match anche per nome (anti-duplicati). Da deployare CF + Vercel + test.
...
- Bonus: shortcuts nel manifest (pressione lunga icona → "Traccia" / "Scansiona"); eventuale `?action=cerca`.
