...
| Cloud Function + tutti gli endpoint (`/log_meal`, `/daily_summary`, `/dashboard`, `/config`, `/health`, `/scan_barcode`, `/catalog`, `/search`, `/log_catalog`, `/update_meal`, `/delete_meal`) | ✅ (CF deployata 2026-07-22, rev `voicetrack-00025-sic`) |
...
| Scan: EAN a mano con microfono (dettatura cifre → campo) | ✅ (codice su `main`; da verificare su telefono + Vercel) |
...
| Edit/delete pasti nella PWA (tap→modifica, pulsante+swipe→elimina; conferma su barra rossa inline, non tendina; pannello edit con animazione collapse altezza/opacità) | ✅ (CF live; frontend su `main` — **Vercel + test telefono**) |
