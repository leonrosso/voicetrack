...
- **PWA:** `https://voicetrack-chi.vercel.app/`; URL backend + chiave in localStorage. CORS aperto (`*` — barriera = API key). Scan barcode: anche `vt-scan-camera-id` (deviceId back camera preferito). Cache anti cold-start: `vt-cache` (oggi + trend + target); giorni ≠ oggi: `vt-day-cache` (max 40 date, SWR).
...
| Diario swipe illimitato passato/futuro + schede colorate (passato caldo / oggi verde / futuro slate) | ✅ (codice PWA; SWR + `vt-day-cache` + fetch `/day_meals`; **da deployare Vercel**) |
...
| PWA completa: dashboard, Config, voce (Deploy 3), barcode (Deploy 4), warm-up `/health` (Diario+Traccia+Scan), cache anti cold-start, carosello swipe animato tra tab Diario/Traccia/Scan | ✅ |
