...
| Scan: paste link scheda OFF + Share Target PWA | ✅ (codice; campo sotto Inquadra; `parseOffBarcode` → `/scan_barcode`; manifest `share_target` + `?action=off`; **da deployare Vercel** + test PWA installata / OFF Condividi) |
...
| Diario: calendario jump sulla card calorie (Settimana/Mese → `goToDate`) | ✅ (codice; icona accanto al titolo giorno; **da verificare su telefono**) |
...
- Bonus: shortcuts nel manifest (pressione lunga icona → "Traccia" / "Scansiona"); eventuale `?action=cerca` (già supportato). **Share Target** OFF → `?action=off` ✅ (codice; da deployare + verificare su PWA installata).
