...
| Cloud Function + tutti gli endpoint (`/log_meal`, `/daily_summary`, `/day_meals`, `/dashboard`, `/config`, `/health`, `/scan_barcode`, `/catalog`, `/search`, `/log_catalog`, `/update_meal`, `/delete_meal`) | ✅ (codice: TargetHistory + `apply_target_span`; `/dashboard` con `weeks_by_offset` + `week_only`; `APP_VERSION` `deploy5-weeks-prefetch-2026-07-24`; **CF da redeployare**) |
...
| Card trend Diario con viste Anno / Mese / Settimana (`storico_annuale` / `storico_mensile` / `storico_settimanale`) | ✅ (codice; pager con `weeks_by_offset` prefetch 12 sett. + `week_only` su miss; **CF da redeployare** + refresh PWA) |
...
- **Card trend multi-range:** ✅ (codice) Anno / Mese / Settimana; dual-view; frecce con prefetch `weeks_by_offset` (12 finestre) + `week_only=1` oltre cache. **Da redeployare CF** (`deploy5-weeks-prefetch-2026-07-24`) + refresh PWA.
