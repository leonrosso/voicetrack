...
DATABASE — Google Sheets (tab dati `diethropic` + tab `Config` + tab `Catalogo` + tab `TargetHistory`)
  Pasti: timestamp | alimento | grammi | kcal | proteine | carboidrati
  | grassi | fonte (tasker-voce / pwa-voce / pwa-testo / pwa-barcode / pwa-catalogo) | note | id | data_dichiarata | tipo_pasto
  Catalogo: id | nome | alias | barcode | kcal/P/C/G per 100g | fonte | off_code | volte | ultimo_uso | preferito
  TargetHistory: valid_from | kcal | proteine | carboidrati | grassi (append-only; target del giorno D = ultima fascia con valid_from ≤ D)
```
...
- **Sheets:** foglio condiviso col service account; tab `diethropic` + tab `Config` (auto-creato, A1:B4 = target corrente) + tab `TargetHistory` (auto-creato al primo accesso config/dashboard: `valid_from|kcal|P|C|G`, backfill dalla prima data pasti) + tab `Catalogo` (auto-creato al primo `/catalog` o `/search` / scan ok). Schema pasti **A–L**: `timestamp` (quando registrato) … `id` (J) + **`data_dichiarata` (K)** = giorno del pasto (coincide con timestamp se non specificato; mezzogiorno del giorno scelto se backdatato) + **`tipo_pasto` (L)** = colazione|pranzo|spuntino|cena se dichiarato a voce/client, altrimenti vuoto (in lettura → euristica sull’ora). Righe legacy senza K/L: si usa il timestamp / fallback ora. **Operativo:** intestazioni `data_dichiarata` in K1 e `tipo_pasto` in L1 sul foglio.
- **PWA:** `https://voicetrack-chi.vercel.app/`; URL backend + chiave in localStorage. CORS aperto (`*` — barriera = API key). Scan barcode: anche `vt-scan-camera-id` (deviceId back camera preferito). Cache anti cold-start: `vt-cache` (oggi + trend + target); giorni ≠ oggi: `vt-day-cache` (max 40 date, SWR, include `target` del giorno). Statistiche: `vt-stats-mode` = `current`|`history`. Scan: campo paste link OFF; Share Target path **`/share-off`** (+ legacy `?action=off`). Manifest: `scope: '/'`; shortcuts pressione lunga **Traccia / Scansiona / Cerca / Testo**; **`share_target`** GET `/share-off`. Solo PWA installata; dopo cambio manifest **reinstallare** l’app.
...
| Cloud Function + tutti gli endpoint (`/log_meal`, `/daily_summary`, `/day_meals`, `/dashboard`, `/config`, `/health`, `/scan_barcode`, `/catalog`, `/search`, `/log_catalog`, `/update_meal`, `/delete_meal`) | ✅ (codice: TargetHistory + `apply_target_span` from/day/range; `/dashboard?week_offset`; `APP_VERSION` `deploy5-week-offset-2026-07-24`; **CF da redeployare**) |
...
| Log vocale con tipo pasto dichiarato («a cena», «a colazione», …) + fallback ora se omesso | ✅ (codice; LLM + colonna L `tipo_pasto`; **CF da redeployare** + header L1) |
| Diario swipe illimitato passato/futuro + schede colorate (passato caldo / oggi verde / futuro slate) + righe pasto stesso surface/line | ✅ (codice PWA; SWR + `vt-day-cache` + `/day_meals` con **target del giorno**; **swipe giorno a tutta larghezza** del `dayPager`; tab solo via bottoni; **da deployare Vercel**) |
...
| Card trend Diario con viste Anno / Mese / Settimana (`storico_annuale` / `storico_mensile` / `storico_settimanale`) | ✅ (codice; dual-view ATTUALE/STORICO; **pager settimane** `week_offset`; gate `weekOk` corretto + anti-flash poll/animazione; **da deployare CF+Vercel**) |
...
| Config con % macro nella PWA | ✅ (codice; Obiettivi ancorati al giorno Diario: **Solo questo giorno** / **Da questo giorno in poi** / **Intervallo** a calendario; `apply_target_span`; **da deployare**) |
...
- **Schema Sheets = moltiplicatore:** i consumatori pasti sono ≥6 (`/log_meal`, `/scan_barcode`, `/daily_summary`, `/day_meals`, `/dashboard`, `/config`); il **Catalogo** è una tab separata (`/catalog`, `/search`, `/log_catalog`) e non allunga lo schema A–L dei pasti. **Ogni colonna aggiunta ai pasti si aggiorna in tutti i consumatori insieme, nella stessa sessione.** Nessuna validazione numerica: una riga malformata inquina i totali in silenzio. `pasto` in lettura: colonna L `tipo_pasto` se valorizzata, altrimenti euristica sull’orario di **`data_dichiarata`** (fallback timestamp); i backdate a mezzogiorno senza tipo dichiarato finiscono spesso in «pranzo».
...
- **Config collegata:** ✅ (codice) editor Obiettivi ancorato al giorno Diario: ambiti Solo giorno / Da quel giorno in poi / Intervallo (calendario range); POST `/config` con `mode`+`start`[+`end`]; bozza da `dayTarget`. Deploy CF+Vercel da fare.
...
- **Card trend multi-range:** ✅ (codice) Anno / Mese / Settimana; dual-view ATTUALE/STORICO; vista Settimana con **frecce** e `GET /dashboard?week_offset=N` (0 = ultimi 7 gg). Fix gate `weekOk` (CF vecchia: solo offset 0) + skip setState se serie invariata + barre senza animazione. **Da deployare CF + Vercel.**
...
- "ieri 200 grammi di yogurt" → K = ieri (12:00), A = adesso; L vuoto → gruppo pranzo (euristica); TTS inizia con «Ieri hai mangiato…».
- "ieri a cena 200 grammi di yogurt" → K = ieri, L = `cena` → gruppo Cena (non pranzo); TTS giorno ok.
- "a colazione un uovo" (anche fuori fascia oraria) → L = `colazione`.
