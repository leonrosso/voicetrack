...
| Cloud Function + tutti gli endpoint (`/log_meal`, `/daily_summary`, `/day_meals`, `/dashboard`, `/config`, `/health`, `/scan_barcode`, `/catalog`, `/search`, `/log_catalog`, `/update_meal`, `/delete_meal`) | ✅ (codice: TargetHistory + `apply_target_span` from/day/range; `/dashboard?week_offset`; **CF da redeployare**) |
...
| Card trend Diario con viste Anno / Mese / Settimana (`storico_annuale` / `storico_mensile` / `storico_settimanale`) | ✅ (codice; dual-view ATTUALE/STORICO; **pager settimane** `week_offset`; **da deployare CF+Vercel**) |
...
- **Card trend multi-range:** ✅ (codice) Anno / Mese / Settimana; dual-view ATTUALE/STORICO; vista Settimana con **frecce** e `GET /dashboard?week_offset=N` (0 = ultimi 7 gg). **Da deployare CF + Vercel.**
