...
| Cloud Function + tutti gli endpoint (`/log_meal`, `/daily_summary`, `/day_meals`, `/dashboard`, `/config`, `/health`, `/scan_barcode`, `/catalog`, `/search`, `/log_catalog`, `/update_meal`, `/delete_meal`) | ✅ (codice: TargetHistory + `apply_target_span`; `/dashboard?week_offset` alleggerito anti-503; `APP_VERSION` `deploy5-dash-light-2026-07-24`; **CF da redeployare**) |
...
| Card trend Diario con viste Anno / Mese / Settimana (`storico_annuale` / `storico_mensile` / `storico_settimanale`) | ✅ (codice; dual-view; pager `week_offset` + retry 503 + UI optimistic + asse giorno-mese; **da deployare CF+Vercel**) |
...
- **Card trend multi-range:** ✅ (codice) Anno / Mese / Settimana; dual-view ATTUALE/STORICO; vista Settimana con **frecce** e `GET /dashboard?week_offset=N`. Dashboard senza write Config + senza `target_history` in payload (anti-503); PWA retry/optimistic/asse. **Da deployare CF + Vercel.**
